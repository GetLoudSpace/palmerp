...
story.append(Paragraph("Tu repo base es <b>template</b>. Cliente hace <b>fork privado → clone → .env → provision → Vercel import</b>. Tu base es <b>upstream</b>: <b>git remote add upstream https://github.com/Palm-ERP/palmerp.git</b>. Cuando tú haces <b>git push origin main</b> en palmerp, cada cliente hace <b>git fetch upstream && git merge upstream/main && git push origin main</b> y su Vercel despliega. El <b>control.palmerp.es</b> solo ve <b>heartbeats</b> (no sus datos).", sBody))
# fleet table visual
fleet_vis = [
...
story.append(Paragraph("Cuando publiques mejora: <b>git fetch upstream && git merge upstream/main --no-edit && git push origin main</b> → Vercel cliente despliega", sBody))
# 5.3
...
story.append(Paragraph("vercel.com → Add New Project → Import cliente-x → Build prisma generate && next build → Add Env (las de 5.5 Production) → Deploy → Visit → Domains → Add cliente.es → Cron Jobs verifica 30 2 * * * /api/cron/daily-backups → git commit --allow-empty -m \"test\" && git push origin main → Deployments verde", sBody))
# 5.8
...
story.append(Paragraph("Tú publicas: <b>package.json version 0.1.0→0.2.0 + git push origin main en Palm-ERP/palmerp</b>. Cliente ve en <b>/admin/settings/updates</b> via <b>GET /api/admin/updates → src/lib/fleet/upstream.ts</b> (GitHub API o git ls-remote, git rev-list --count, semver) → <b>N commits por detrás</b> + backup age.", sBody))
story.append(Paragraph("Vercel (7 cmds copia/pega):", sH3))
story.append(Paragraph("git fetch upstream<br/>git merge upstream/main --no-edit --no-ff<br/>npm ci --legacy-peer-deps<br/>npx prisma generate<br/>npx prisma migrate deploy<br/>npm run build<br/>git push origin main  # Vercel 1-2 min", sCode))
