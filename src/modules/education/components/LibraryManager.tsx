    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex gap-2 overflow-x-auto">
          <button onClick={()=>setInstr("ALL")} className={`shrink-0 rounded-full border px-3 py-1 text-xs font-bold ${instr==="ALL"?"bg-amber-500 text-white border-amber-500":"bg-background"}`}>Todos</button>
          {Object.values(EDUCATION_INSTRUMENTS).filter(i=>i.key!=="COMMON").map((ins)=><button key={ins.key} onClick={()=>setInstr(ins.key)} className={`shrink-0 rounded-full border px-3 py-1 text-xs font-bold ${instr===ins.key?"bg-amber-500 text-white":"bg-background"}`}>{ins.label}</button>)}
        </div>
      </div>
      ...
      {tab==="exercises" && (
        <div className="space-y-3">
          <div className="rounded-3xl border border-border/40 bg-card p-4 shadow-sm">
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-amber-600"><Icons.Search className="h-4 w-4" /> Buscar ejercicios (web)</div>
            ...
            <div className="mt-3 flex gap-2">
              <input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="ej. cejilla guitarra nivel medio, slap bajo, voz respiración..." className="flex-1 rounded-xl border border-border/40 bg-background px-3 py-2.5 text-sm outline-none focus:border-amber-500" />
              ...
              <div className="mt-2 grid gap-2">
                {candidates.map((c)=>(
                  <div key={c.id} className="rounded-2xl border border-border/40 bg-card p-3">
                    <div className="flex items-start justify-between gap-2"><div><div className="text-sm font-bold">{c.title}</div><div className="text-[11px] text-muted-foreground flex flex-wrap gap-1 items-center"><span className="rounded-full bg-muted px-2 py-0.5">{c.instrument}</span><span className="rounded-full bg-muted px-2 py-0.5">{c.level}</span><span className="rounded-full bg-amber-500/10 px-2 py-0.5 text-amber-700">Dif {c.difficulty}/5</span><span>{c.estimatedMin}′</span><span className="rounded-full border px-2 py-0.5">{c.sourceType}</span></div><div className="mt-1 text-xs">{c.description}</div><pre className="mt-1 overflow-auto rounded-lg bg-muted p-2 text-[11px] font-mono">{c.tabContent}</pre><a href={c.sourceUrl} target="_blank" rel="noreferrer" className="text-[11px] text-amber-600 underline">{c.sourceUrl}</a></div><button onClick={()=>saveCandidate(c)} className="shrink-0 rounded-xl bg-amber-500 px-3 py-2 text-xs font-bold text-white">Guardar</button></div>
                  </div>
                ))}
              </div>
            </div>
