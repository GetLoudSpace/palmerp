  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-border/30 pb-4">
        <div>
          <div className="text-xl font-extrabold tracking-tight text-foreground md:text-2xl flex items-center gap-2">
            <Icons.RefreshCw className="h-6 w-6 text-red-600" />
      ...
      {data && (
        <>
          {/* Semáforo */}
          <div className={`rounded-2xl border p-5 ${data.updateAvailable ? "border-red-500/25 bg-red-500/10" : "border-emerald-500/25 bg-emerald-500/10"}`}>
            <div className="flex items-start gap-3">
              <div className={`rounded-xl p-2 ${data.updateAvailable ? "bg-red-500/20 text-red-700" : "bg-emerald-500/20 text-emerald-700"}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                {data.behindByCommits !== null && data.behindByCommits > 0 && (
                  <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-red-500/15 px-3 py-1 text-xs font-bold text-red-800 dark:text-red-200">
                    <Icons.GitCommit className="h-3.5 w-3.5" /> {data.behindByCommits} commit(s) por detrás
                  </div>
                )}
              </div>
              <span className={`shrink-0 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ${data.updateAvailable ? "bg-red-600 text-white" : "bg-emerald-600 text-white"}`}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-border/40 bg-card p-5">
              <div className="text-xs font-bold uppercase tracking-widest text-red-600">Tu instancia (local)</div>
          ...
          <div className="rounded-2xl border border-border/40 bg-card p-6">
            <div className="text-sm font-black text-foreground flex items-center gap-2"><Icons.Zap className="h-4 w-4 text-red-600" /> Actualizar ahora</div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <button onClick={load} className="inline-flex items-center gap-1.5 rounded-xl bg-red-600 px-3 py-2 text-xs font-bold text-white hover:bg-red-700">
                <Icons.RefreshCw className="h-4 w-4" /> Volver a comprobar
              </button>
            </div>

            <div className="mt-4 rounded-xl bg-red-500/10 border border-red-500/20 p-3 text-xs leading-5 text-red-900 dark:text-red-100">
